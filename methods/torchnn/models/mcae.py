...
class mcae(nn.Module):
    def __init__(self, in_c=1, num_classes=1, dim=27):
        ...
        self.dconvblock1 = nn.Sequential(
            nn.ConvTranspose1d(96, 48, 1),
            nn.BatchNorm1d(48),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(48, 48, 3),
            nn.BatchNorm1d(48),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(48, 48, 5),
            nn.BatchNorm1d(48),
            nn.ReLU(inplace=True)
        )

        self.dconvblock2 = nn.Sequential(
            nn.ConvTranspose1d(48, 24, 1),
            nn.BatchNorm1d(24),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(24, 24, 3),
            nn.BatchNorm1d(24),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(24, 24, 5),
            nn.BatchNorm1d(24),
            nn.ReLU(inplace=True)
        )

        self.dconvblock3 = nn.Sequential(
            nn.ConvTranspose1d(24, 1, 1),
            nn.BatchNorm1d(1),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(1, 1, 3),
            nn.BatchNorm1d(1),
            nn.ReLU(inplace=True),

            nn.ConvTranspose1d(1, 1, 5),
            nn.BatchNorm1d(1),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        x = self.dconvblock1(x)
        # print(x.shape)
        x = self.dconvblock2(x)
        # print(x.shape)
        x = self.dconvblock3(x)
        # print(x.shape)
        return x
